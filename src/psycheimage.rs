...
impl PsycheImage {
    ...
    pub fn open(file_path: &str, instrument: enums::Instrument) -> Self {
        ...
        PsycheImage {
            ...
            instrument,
            metadata: PsycheImage::load_image_metadata(file_path),
            empty: false,
    ...
    pub fn open_dct_coefficient_fix(file_path: &str, instrument: enums::Instrument) -> Self {
        ...
        if match Self::is_jpeg(file_path) {
            ...
            PsycheImage::open(file_path, instrument)
        }
    }

    fn load_image_metadata(file_path: &str) -> Metadata {
        let metadata_file = util::replace_image_extension(file_path, ".json");
    ...
    pub fn flatfield_with_flat(&mut self, flat: &PsycheImage) {
    ...
    fn compute_destretch_lut(&self) -> DnVec {
        ...
        (0..255).for_each(|i| {
            if PsycheImage::is_index_a_histogram_gap(&hist, i) {
                value_minus += 1.0;
            }
    ...
    pub fn destretch_image(&mut self) {
        ...
        (0..self.image.num_bands()).for_each(|i| {
            self.image.set_band(
                &PsycheImage::destretch_buffer_with_lut(self.image.get_band(i), &lut),
                i,
            );
        });
    }
